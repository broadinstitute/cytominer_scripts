...
write_gct <- function(df, fname) {
  stopifnot(stringr::str_detect(fname, "gct$"))
  ...
  n_readouts <- nrow(measurements)
  
  n_samples <- ncol(measurements)
  
  n_row_annot <- ncol(row_annotations)

  n_col_annot <- ncol(column_annotations) - 1 # id is not counted
  ...
  readr::write_tsv(x = column_annotations_df, 
                   path = fname, append = TRUE, col_names = FALSE)

  readr::write_tsv(x = measurements_df, path = fname, append = TRUE)
