...
backend_dir <- paste("../..", "metadata", batch_id, plate, sep = "/")

profiles <- read_csv(paste(backend_dir, paste0(plate_id, ".csv"), sep = "/"))

# read profiles
cnames <- colnames(profiles)

cnames %<>% str_replace_all("^Image_Metadata", "Metadata")

names(profiles) <- cnames

# read and join metadata map

metadata_map <- read_csv(paste(metadata_dir, "barcode_platemap.csv", sep = "/"))

assert "Assay_Plate_Barcode" %in% colnames(metadata_map)

cnames <- colnames(metadata_map)

cnames %<>% str_replace_all("^", "Metadata_")
...
expect_equal(length(cpd_map_name), 1)

platemap <- read_tsv(paste(metadata_dir, "platemap_cpd", paste0(cpd_map_name, ".txt"), sep = "/"))

assert "well_position " %in% colnames(platemap)
...
profiles %>% write_csv(profiles_with_metadata)
